...
class Web3DViewerWeb extends Web3DViewer {
  ...
  Widget _buildSingleModel3D(BuildContext context) {
    final String viewType = 'model-viewer-${modelUrl.hashCode}-${frontImage.hashCode}-${DateTime.now().millisecondsSinceEpoch}';
...
<body>
  ...
  <script>
    ...
    loader.load('$resolvedUrl', (gltf) => {
      ...
      const centeredBox = getPercentileBoundingBox(model);
...
class ThreeDWebRoomViewerWeb extends StatefulWidget {
  ...
  @override
  State<ThreeDWebRoomViewerWeb> createState() => _ThreeDWebRoomViewerWebState();
}

class _ThreeDWebRoomViewerWebState extends State<ThreeDWebRoomViewerWeb> {
...
<body>
  ...
  <script>
    ...
    loader.load('assets/models/' + glbName, (gltf) => {
      ...
      console.warn("Could not load assets/models/" + glbName + ", generating fallback.");
